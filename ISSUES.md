...
## Additional Bugs (from component-level review)

### BUG-6: `_moveFocusByMonths` does not skip disabled dates

**Severity:** Medium
**File:** `src/plugin/calendar-component.ts`, lines 2171-2174
...
Unlike `_moveFocus()` (which skips disabled dates with up to 31 attempts), `_moveFocusByMonths()` sets the focused date directly without checking if it's disabled. Pressing Page Up/Page Down can land the focus on a disabled date. While `selectDate()` blocks actual selection of disabled dates, the visual focus ring sits on a disabled cell, which is confusing for keyboard users and a violation of expected ARIA grid navigation behavior.

**Fix:** Add disabled-date skipping logic similar to `_moveFocus()`, or at minimum find the nearest non-disabled date in the target month.

---

### BUG-7: Scroll observer misses initial render in scrollable mode

**Severity:** Low
**File:** `src/plugin/calendar-component.ts`, lines 908-909
...
The scroll observer is initialized in `$nextTick`, but the auto-rendered template (lines 840-847) uses `Alpine.initTree()` which processes `x-for` and `x-if` directives that may themselves use `$nextTick` to render. The `[data-month-id]` elements generated by `x-for` may not exist in the DOM when `_initScrollListener` runs, causing `querySelectorAll('[data-month-id]')` to find 0 elements and return early.

The observer eventually gets set up on the first month/year navigation (via `_rebindScrollObserver` in the `$watch` callbacks), but the **initial** render has no scroll observer — the sticky header label won't update as the user scrolls until they navigate.

**Fix:** Use a double `$nextTick` or a `requestAnimationFrame` to ensure the DOM is fully rendered before initializing the observer, or use a MutationObserver to detect when the month elements appear.
