...
- [x] Recurring rules: `{ months: [1,2,3,4] }` to apply rules to specific months every year without specifying exact date ranges; `DateConstraintRule.months` field for core, `CalendarConfigRule.months` for Alpine config; `matchesRule()` helper checks both date-range and recurring-month matching; first-match-wins preserved; works with all constraint properties, range validation, and `updateConstraints()`; 16 core tests in constraints.test.ts + 6 component tests in calendar-component.test.ts (1091 total)
